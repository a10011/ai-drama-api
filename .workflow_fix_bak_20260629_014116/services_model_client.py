...
def _generate_image_with_model(
    ...
    try:
        ...
        urls: List[str] = []
        # 竖屏构图 + 风格控制（模型感知：wanxiang不用negative_prompt，靠纯正面词）
        if model_name in ("wanxiang", "wan2.7-image-pro"):
            framed_prompt = (
                "hyperrealistic live-action film still, real photograph shot on ARRI Alexa, "
                "realistic human with natural skin pores, realistic clothing fabric texture, "
                "cinematic lighting professional cinematography photorealistic 8K, "
                "realistic environment natural lighting, live action movie scene, "
                "NOT cartoon NOT anime NOT 3D animation NOT illustration NOT painting NOT drawing NOT CGI NOT game NOT stylized, "
                "photorealistic real photography real human real environment, "
                "不是卡通不是动漫不是3D动画不是插画不是手绘不是游戏风格, "
                "写实照片真实摄影真人实拍, "
                "vertical portrait 9:16 full body centered fill frame --- "
                + prompt
            )
        ...
        if model_name == "seedream":
            ...
            urls = provider.generate_image(framed_prompt, size, timeout=timeout)
            if not urls:
        ...
        if _should_retry(category, retry_state.attempt if retry_state else 0):
            ...
            return _generate_image_with_model(model_name, prompt, size, timeout, retry_state, order_id=order_id, user_id=user_id, drama_id=drama_id)
    ...
    **kwargs
    ...
    try:
        ...
        if model_name == "wan2.7_t2v":
            ...
            urls = provider.generate_video(prompt, image_url, resolution=resolution, motion_intensity=kwargs.get("motion_intensity", 0.5))
            ...
            result = provider.generate_video(prompt, duration=kwargs.get("duration", 5), max_wait=timeout or 600)
            video_url = ""
            if isinstance(result, dict):
            ...
            if hh_mode == "r2v":
                ...
                if char_img and char_img not in ref_images:
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            if not _acquire_model_slot(model_name, timeout=slot_timeout):
                retry_count += 1
                wait = min(10 * (2 ** retry_count), 120)
                logger.warning(f"[ModelClient] {model_name} 槽位满 → {wait}s后重试 ({retry_count}/{MAX_RETRIES})")
                _time.sleep(wait)
                raise RuntimeError("单模型终止，无备用")
            
            try:
                retry_state = RetryState(model_name=model_name)
                urls = _generate_image_with_model(model_name, prompt, final_size, final_timeout, retry_state, order_id=order_id, user_id=user_id, drama_id=drama_id)
            finally:
                _release_model_slot(model_name)
            
            if urls and len(urls) > 0:
                logger.info(f"[ModelClient] ✅ {model_name} 成功 (order={order_id}, retries={retry_count})")
                return ModelResult(success=True, url=urls[0], model=model_name, error="", order_id=order_id)
            
            # 返回空 → 同模型重试，不降级
            retry_count += 1
            wait = min(5 * (2 ** min(retry_count, 5)), 120)
            logger.warning(f"[ModelClient] {model_name} 返回空 → {wait}s后重试 ({retry_count}/{MAX_RETRIES})")
            _time.sleep(wait)
            ...
            # 鉴权失败(401/403) → 模型永久不可用，立刻试下一个
            if "401" in error_str or "403" in error_str or "InvalidApiKey" in error_str or "Unauthorized" in error_str:
                logger.error(f"[ModelClient] {model_name} 鉴权失败 → 切换模型: {error_str[:80]}")
                raise  # 单模型，直接抛异常
            
            # 限流(429/rate/限流/overloaded) → 等久点，继续同模型
            if any(k in error_str.lower() for k in ("429", "rate", "limit", "throttl", "限流", "overload", "busy")):
                retry_count += 1
                wait = min(15 * (2 ** min(retry_count, 4)), 120)  # 限流等更久: 15s→30s→60s→120s
                logger.warning(f"[ModelClient] {model_name} 限流 → {wait}s后重试 ({retry_count}/{MAX_RETRIES})")
                _time.sleep(wait)
                raise RuntimeError("单模型终止，无备用")
            
            # 其他错误(5xx/超时/网络) → 短等后重试同模型
            retry_count += 1
            wait = min(5 * (2 ** min(retry_count, 5)), 60)
            logger.warning(f"[ModelClient] {model_name} 异常({_classify_error_str(error_str)}) → {wait}s后重试 ({retry_count}/{MAX_RETRIES}): {error_str[:60]}")
            _time.sleep(wait)
    
    # 该模型重试耗尽
    logger.error(f"[ModelClient] {model_name} {MAX_RETRIES}次重试失败 → 终止")
    # model_manager disabled (单模型OK)
    
    return ModelResult(success=False, error="所有模型均失败(重试耗尽)", url="", model="")
    ...
    **kwargs
    ...
    if not cfg or cfg.get("static"):
        ...
        try:
            ...
            retry_count = 0
            max_same_model_retries = 2  # 同模型重试2次，不跨模型降级  # 零重试
            ...
            logger.error(f"[ModelClient] ⚠️ {model_name} 视频 {max_same_model_retries}x 重试仍空")
    ...
    if not cfg or cfg.get("static"):
        ...
        try:
            ...
            if model_name == "edge-tts":
                data = provider.synthesize(text, voice, timeout=final_timeout)
            else:
...
def call_llm(
    ...
    try:
        ...
        if registry_model:
            ...
            _PROV_CLASSES = {}
            try:
                from services.ai_providers import DoubaoProvider as _DP, QwenProvider as _QP, DeepSeekProvider as _DSP, ZhipuProvider as _ZP
                _PROV_CLASSES = {"DoubaoProvider": _DP, "QwenProvider": _QP, "DeepSeekProvider": _DSP, "ZhipuProvider": _ZP}
            except ImportError:
...
def generate_image_to_image(
    ...
    if reference_image and reference_image.startswith('/www/wwwroot/'):
        reference_image = 'https://ai.mzsh.top/' + reference_image[len('/www/wwwroot/'):]
    ...
    # ── 链1: Seedream (ARK/火山) — 有style=photography控制，优先使用 ──
    try:
        from services.ai_providers import ARKImageProvider
        ark = ARKImageProvider()
        norm_size_seedream = _normalize_size(size, "seedream")
        # seedream I2I strength mapping: 0.08→0.3(保脸), 0.25→0.45, 0.50→0.65
        sd_strength = max(0.2, min(0.8, strength_val * 1.5))
        urls = ark.generate_image_to_image(framed_i2i_prompt, reference_image, norm_size_seedream, strength=sd_strength)
        if urls and len(urls) > 0:
            logger.info(f"[ModelClient] ✅ seedream I2I (strength={sd_strength:.2f}) url={urls[0][:80]}")
...
def download_to_storage(
    ...
    try:
        ...
        fig_path = os.path.join(fig_dir, f"{prefix}{safe_name}_{h}.jpg")
        
        if os.path.exists(fig_path) and os.path.getsize(fig_path) > 100:
            return f"/storage/{pipeline_id}/figures/{os.path.basename(fig_path)}" if pipeline_id else "/storage/figures/" + os.path.basename(fig_path)
...
class UnifiedModel:
    ...
    def image(
        ...
        return generate_image(prompt, preferred, size, timeout, project_id=project_id, user_id=0, drama_id="")
    ...
    def download_to_storage(
        ...
        return download_to_storage(url, name_hint, user_id, order_id, pipeline_id=self._pipeline_id)