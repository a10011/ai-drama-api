...
def _read_scene_map(project_id: str) -> dict:
    ...
    if not img_map:
        # 兼容 scene_images list 格式 [{image_url: ...}, ...]
        scene_list = data.get("scene_images", [])
        if isinstance(scene_list, list) and scene_list:
            img_map = {}
            for i, si in enumerate(scene_list):
                if isinstance(si, dict):
                    url = si.get("image_url", si.get("url", ""))
                    if url:
                        img_map[str(i)] = url
...
def inject_shot_media(shots: list, project_id: str, include_tts: bool = True) -> dict:
    ...
    for i, shot in enumerate(shots):
        ...
        # 统一匹配文本：description + dialogue + scene
        match_text = ""
