...
返回JSON格式（不要markdown代码块）：
{
  ...
  "props": ["道具1", "道具2"],
  "scene_prompt": "完整的AI绘图提示词（80-150字，丰富细节。必须包含：写实电影质感、真实摄影、竖屏9:16构图主体偏上下方留白、环境细节（建筑/家具/自然元素）、光线（方向+性质+色温）、氛围（情绪+温度感）、色彩基调、景深层次（前景/中景/背景）。补充英文电影摄影关键词如 cinematic lighting, depth of field, volumetric。严禁卡通、动漫、手绘风格）"
}"""
...
_CINEMATIC_TAGS = {
    "修仙": "cinematic photography, ethereal mist, floating particles, ancient Chinese architecture, volumetric lighting, warm amber tones, dreamlike atmosphere, 8K",
    "仙侠": "cinematic photography, ethereal mist, floating particles, ancient Chinese architecture, volumetric lighting, warm amber tones, dreamlike atmosphere, 8K",
    "古装": "cinematic period drama, ancient Chinese architecture, warm natural light, rich textures, silk fabrics, traditional decor, depth of field, 8K",
    "武侠": "cinematic wuxia style, dramatic lighting, misty mountains, traditional architecture, warm ambient light, bamboo forest atmosphere, 8K",
    "都市": "cinematic urban, modern architecture, natural window light, clean lines, realistic textures, warm mood, shallow depth of field, 8K",
    "科幻": "cinematic sci-fi, neon lighting, high contrast, metallic textures, volumetric fog, cool blue tones, ultra realistic, 8K",
    "悬疑": "cinematic noir, dramatic shadows, low key lighting, moody atmosphere, desaturated colors, rain and reflections, 8K",
    "恐怖": "cinematic horror, dark atmosphere, cold blue-green tones, high contrast shadows, damp textures, eerie lighting, 8K",
}

def _enrich_prompt_with_style(prompt, genre, director_task=""):
    """根据剧集类型给场景 prompt 注入时代风格。
    现代题材：原始描述已足够，不加风格标签避免污染。
    古装/仙侠等：注入对应时代风格。"""
    # genre 永不落空
    ...
    # 现代题材：描述本身已足够，只加画质标签
    modern_genres = ("现代", "都市", "甜宠", "职场", "商战", "悬疑", "科幻", "恐怖", "逆袭", "重生", "复仇", "军旅", "乡村", "校园", "家庭", "搞笑")
    if genre in modern_genres or any(mg in genre for mg in modern_genres):
        if len(prompt) < 80:
            prompt += "，modern interior，natural light，realistic texture"
        return prompt + "，cinematic lighting，photorealistic，8K，real photography"
    ...
    enriched = prompt + sep + extra

    # Append cinematic photography tags by genre
    for key_word, tags in _CINEMATIC_TAGS.items():
        if key_word in genre:
            enriched += "， " + tags
            break
...
class SceneAgent(BaseAgent):
    ...
    def design_scene(self, scene_name: str, script_context: str, model_routes: dict = None) -> AgentResult:
        self.report_progress(f"分析场景需求: {scene_name}", 10)
        start = time.time()
        try:
    ...
    def generate_scene_image(self, scene_prompt: str, director_task: str = '', reference_image: str = '', genre: str = '', project_id: int = 0) -> AgentResult:
        ...
        if reference_image and (reference_image.startswith('http') or reference_image.startswith('/storage/')):
            ...
            if reference_image.startswith('/storage/'):
                reference_image = 'https://ai.mzsh.top' + reference_image
            try:
        ...
        if result and result.get("success"):
            ...
            if os.path.exists(fb) and os.path.getsize(fb) > 100:
                ...
                return AgentResult(
                    data={"image_url": "https://ai.mzsh.top" + fb.replace("/www/wwwroot", ""), "model": "ffmpeg", "prompt": scene_prompt},
                    duration_ms=int((time.time()-start)*1000),
                    success=True
                )
    ...
    def run(self, action: str = "design", **kwargs) -> AgentResult:
        ...
        if isinstance(_dt, dict) and _dt.get("scene_generation"):
            kwargs["director_task"] = str(_dt["scene_generation"])
        elif isinstance(_da, dict) and _da.get("director_vision"):
            kwargs["director_task"] = "画风：" + str(_da["director_vision"])
        if not kwargs.get("genre"):
            _g = kwargs.get("params", {}).get("genre", "") or (kwargs.get("director_genre", "") if isinstance(kwargs.get("director_genre"), str) else "")
            if _g: kwargs["genre"] = _g
        ...
        if action == "design" or action == "generate":
            ...
            if action == "generate" and shot_desc:
                self.report_progress("正在生成场景图...", 50)
    ...
    def execute(self, shot: dict, reference_image: str = "", project_id: int = 0, **kwargs):
        """唯一入口：生成场景图"""
        scene_prompt = shot.get("scene_prompt", shot.get("prompt", ""))
        if not scene_prompt:
            desc = shot.get("description", "")
            ...
            scene_prompt = (desc + "，环境：" + env + "，" + sp)[:200] or ""
        if scene_prompt:
            return self.generate_scene_image(scene_prompt, model_routes, reference_image=reference_image, project_id=project_id)
        return AgentResult(success=False, error="无场景提示词")
