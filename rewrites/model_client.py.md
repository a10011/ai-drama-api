...
MODEL_REGISTRY: Dict[str, ModelConfig] = {
    ...
    "hidream": {
        ...
        "model": "z1-image",
        "type": "image", "size": "1024x1024", "timeout": 60,
    },
    "agnes": {
        "provider": "AgnesAIProvider", "service": "agnes",
        "model": "agnes-image-2.1-flash",
        "type": "image", "size": "1024x1024", "timeout": 30,
    },
    "wanxiang": {
...
ROUTING_CHAINS: Dict[str, List[str]] = {
    "image": ["hidream"],
    "video": ["seedance", "happyhorse", "kling"],
    "face":  ["hidream"],
    "tts":   ["edge-tts"],
...
def generate_image(
    ...
    """图片生成。自动路由链降级，返回结构化结果"""
    chain = ROUTING_CHAINS["image"].copy()
    ...
    for model_name in chain:
        ...
        try:
            ...
            # 空结果处理
            logger.warning(
                f"[ModelClient] ⚠️ {model_name} 返回空 | "
                f"provider={cfg['provider']} | → 降级到下一个模型"
            )
            last_error = f"{model_name}: 返回空列表"
            continue
...
def generate_video(
    ...
    """视频生成。自动路由链降级，返回结构化结果"""
    chain = ROUTING_CHAINS["video"].copy()
    if preferred and preferred in chain:
        chain = [preferred] + [m for m in chain if m != preferred]
    elif preferred and preferred not in chain:
        chain = [preferred] + chain
    ...
    for model_name in chain:
        ...
        try:
            ...
            logger.warning(f"[ModelClient] ⚠️ {model_name} 视频返回空 → 降级")
            last_error = f"{model_name}: 返回空列表"
            continue
...
def generate_image_to_image(
    ...
    try:
        ...
        urls = provider.generate_image_to_image(prompt, reference_image, size)
...
class UnifiedModel:
    ...
    def image_to_image(
        ...
        return generate_image_to_image(prompt, reference_image, size, timeout)
